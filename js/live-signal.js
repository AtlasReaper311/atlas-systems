 ...
 *
 * Homepage authority for live operational state and rendering.
 *
 * Responsibilities:
 * - poll deploy-watch and backend health endpoints
 * - consume atlas-registry snapshots without owning registry acquisition
 * - hold one homepage live-state model
 * - coalesce telemetry DOM work into at most one requestAnimationFrame render pass
 * - pause homepage polling while the document is hidden and refresh on return
 *
 * GitHub activity remains owned by pulse.js, but pulse.js must not write to the
 * deploy, commit, build, backend, navigation-status, or estate-strip surfaces.
 */
(function () {
  "use strict";
  ...
  function scheduleCountdownTick() {
    ...
    const delay = remainder === 0 ? COUNTDOWN_TICK_MS : COUNTDOWN_TICK_MS - remainder;
  ...
  async function fetchService(service) {
    try {
      ...
      state.services[service.key] = { status: response.ok ? "ok" : "error" };
    } catch {
  ...
  function renderDeploy() {
    ...
    if (status === "loading") {
      [lastDeployEl, commitEl, buildEl].forEach((element) => {
    ...
    if (commitEl) {
      if (data.commitUrl) {
        ...
        commitEl.textContent = data.commitSha;
      }
    }
    ...
    if (buildEl) {
      ...
      const config = map[data.status] ?? {
        text: data.status || "—",
        color: "var(--accent)",
      };
      buildEl.textContent = config.text;
      buildEl.style.color = config.color;
    }
  }
  ...
  function subscribeToRegistry() {
    if (!window.AtlasRegistry?.subscribe || registryUnsubscribe) return;

    registryUnsubscribe = window.AtlasRegistry.subscribe((snapshot) => {
