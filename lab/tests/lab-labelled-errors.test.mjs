...
test("LAB-007c labels anomaly fallback diagnostics with stable source and context", () => {
  ...
  assert.match(
    anomaly,
    /console\.warn\(\s*"\[lab\/anomaly\] live evidence load failed; rendering the existing labelled fallback",\s*error,\s*\);/,
  );
...
test("LAB-007c labels conformance fallback diagnostics with stable source and context", () => {
  ...
  assert.match(
    conformance,
    /console\.warn\(\s*"\[lab\/conformance\] live evidence load failed; rendering the existing no-report fallback",\s*error,\s*\);/,
  );
