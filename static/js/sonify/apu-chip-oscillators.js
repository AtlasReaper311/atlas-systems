 ...
 *
 * The LFSR uses the NES 2A03 polynomial: bits 0 and 1 (short) or bits 0
 * and 6 (long) are XORed and fed back into bit 14.
 *
 * @param {BaseAudioContext} ctx
...
export function createLfsrNoiseBuffer(ctx, short = false) {
  ...
  let shift = 1;
  const feedbackBit = short ? 1 : 6;

  for (let i = 0; i < length; i += 1) {
    const sample = i % period;
    if (sample === 0 && i > 0) shift = 1; // reset each period for consistency
