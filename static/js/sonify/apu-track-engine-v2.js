...
} from "./apu-arranger.js?v=20260726-system-symphony-atlas-apu-track-v2";
import {
  APU_TRACK_STEPS,
...
} from "./apu-track-sequencer.js?v=20260726-system-symphony-atlas-apu-track-v2";
...
function setPulseWidth(synth, width, at = undefined) {
  ...
  if (parameter) safeRamp(parameter, clamp(width, 0.08, 0.75), 0.08, at);
}
...
export function createApuTrackEngine({
  ...
  function buildGraph(Tone) {
    ...
    nodes.limiter = new Tone.Limiter(-1.5);
    nodes.compressor = new Tone.Compressor({ threshold: -20, ratio: 2.5, attack: 0.018, release: 0.2 });
    ...
    nodes.primary.chain(nodes.primaryFilter, nodes.primaryBus);
    ...
    nodes.secondary.chain(nodes.secondaryFilter, nodes.secondaryBus);
    ...
    nodes.pad.chain(nodes.padFilter, nodes.padBus);
    ...
    nodes.transition = new Tone.PolySynth(Tone.FMSynth, {
      harmonicity: 2,
      modulationIndex: 4.5,
      oscillator: { type: "sine" },
      modulation: { type: "triangle" },
      envelope: { attack: 0.002, decay: 0.1, sustain: 0.08, release: 0.18 },
      modulationEnvelope: { attack: 0.002, decay: 0.08, sustain: 0.12, release: 0.14 },
      volume: -16,
  ...
  function applyScene(frame, at = undefined) {
    ...
    safeRamp(nodes.transport.bpm, currentArrangement?.targetBpm ?? scene.bpm, 0.8, at);
    safeRamp(nodes.masterVolume.volume, scene.masterGainDb, 0.8, at);
    ...
    safeRamp(nodes.hatFilter.frequency, Math.max(5600, profile.noiseBrightnessHz), 0.5, at);
  }
  ...
  function applyArrangementMix(at = undefined) {
    ...
    safeRamp(nodes.primaryBus.gain, mix.primary, 0.18, at);
    safeRamp(nodes.secondaryBus.gain, mix.secondary, 0.18, at);
    safeRamp(nodes.serviceBus.gain, mix.services, 0.18, at);
    safeRamp(nodes.bassBus.gain, mix.bass, 0.18, at);
    safeRamp(nodes.drumBus.gain, mix.drums, 0.18, at);
    safeRamp(nodes.padBus.gain, mix.pad, 0.18, at);
    safeRamp(nodes.accentBus.gain, mix.accent, 0.18, at);
    ...
    safeRamp(nodes.padFilter.frequency, scene.profile.padCutoffHz * (timbre.padCutoffScale ?? 1), 0.2, at);
  }
  ...
  function playPrimary(time, step) {
    ...
    if (event) nodes.primary.triggerAttackRelease(midiToFrequencyHz(event.midi), event.duration, time, event.velocity);
  }

  function playSecondary(time, step) {
    const event = secondaryPulseEventForTrackStep(currentFrame, currentArrangement, step);
    if (event) nodes.secondary.triggerAttackRelease(midiToFrequencyHz(event.midi), event.duration, time, event.velocity);
  }
  ...
  function playTransition(time, step) {
    ...
    nodes.transition.triggerAttackRelease(midiToFrequencyHz(event.midi), event.duration, time, event.velocity);
  }
  ...
  function onStep(time) {
    ...
    // Advance before rendering. A channel exception can no longer pin the
    // scheduler to one percussion step and produce an endless hi-hat loop.
    stepIndex += 1;
