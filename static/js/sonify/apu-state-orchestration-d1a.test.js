...
test("Candidate B removes automatic phrase arps and keeps D1A disconnected", () => {
  for (const state of ["healthy", "warning", "critical", "unknown"]) {
    const instructions = ornamentInstructionsForPhrase(plan(state, 4));
    assert.equal(instructions.some((event) => event.ornament === "connective-arp"), false, state);
...
test("Candidate B preserves the more modest Pass C Boss mix", () => {
