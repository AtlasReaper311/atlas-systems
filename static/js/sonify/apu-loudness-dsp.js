export const APU_LOUDNESS_DSP_BUILD_ID = "20260726-system-symphony-loudness-dsp-v1";
export const ITU_REFERENCE_SAMPLE_RATE = 48000;
export const LOUDNESS_OFFSET_LKFS = -0.691;
export const ABSOLUTE_GATE_LKFS = -70;
export const RELATIVE_GATE_DB = -10;
export const MOMENTARY_WINDOW_SECONDS = 0.4;
export const SHORT_TERM_WINDOW_SECONDS = 3;
export const GATING_STEP_SECONDS = 0.1;
...
export function powerToLkfs(power) {
...
export function remapBiquadFrom48k(reference, sampleRate = ITU_REFERENCE_SAMPLE_RATE) {
  const targetRate = finitePositive(sampleRate, ITU_REFERENCE_SAMPLE_RATE);
...
export function kWeightingCoefficients(sampleRate = ITU_REFERENCE_SAMPLE_RATE) {
  return Object.freeze({
    stageOne: remapBiquadFrom48k(STAGE_ONE_REFERENCE, sampleRate),
    stageTwo: remapBiquadFrom48k(STAGE_TWO_REFERENCE, sampleRate),
  });
}
...
export class KWeightingFilter {
  constructor(sampleRate = ITU_REFERENCE_SAMPLE_RATE) {
...
export function gatedIntegratedLoudness(blockPowers = []) {
  ...
  if (!gated.length) {
    return Object.freeze({
      integratedLufs: Number.NEGATIVE_INFINITY,
      relativeGateLufs,
      absoluteBlockCount: absolute.length,
      gatedBlockCount: 0,
    });
  }
...
export class StereoLoudnessAccumulator {
  constructor(sampleRate = ITU_REFERENCE_SAMPLE_RATE, { maxBlockHistory = 216000 } = {}) {
    this.sampleRate = finitePositive(sampleRate, ITU_REFERENCE_SAMPLE_RATE);
    ...
    this.lastMetrics = Object.freeze({
      ...
      sessionTruePeakDbtp: Number.NEGATIVE_INFINITY,
      relativeGateLufs: Number.NEGATIVE_INFINITY,
      blockCount: 0,
      gatedBlockCount: 0,
      ready: false,
  ...
  pushEnergy(energy) {
    const outgoingShort = this.energyCount >= this.shortTermSamples
      ...
      : 0;
    const momentaryOutgoingIndex = (this.energyCursor - this.momentarySamples + this.shortTermSamples) % this.shortTermSamples;
...
export function createStereoLoudnessAccumulator(sampleRate, options) {
  return new StereoLoudnessAccumulator(sampleRate, options);
}
