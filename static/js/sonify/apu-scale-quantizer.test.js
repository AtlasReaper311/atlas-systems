...
test("unknown state names fail closed to the unknown scale", () => {
  const q = new ScaleQuantizer({ state: "offline-or-invalid" });
  assert.deepEqual([...q.scale], [0, 2, 5, 7, 10]);
});

test("custom scales are copied and cannot drift after construction", () => {
  const custom = [0, 3, 7];
  const q = new ScaleQuantizer({ scaleOverride: custom });
  custom.push(11);
  assert.deepEqual([...q.scale], [0, 3, 7]);
  assert.equal(Object.isFrozen(q.scale), true);
});

test("empty or invalid custom scales fall back to unknown", () => {
  const empty = new ScaleQuantizer({ scaleOverride: [] });
  const invalid = new ScaleQuantizer({ scaleOverride: [NaN, Infinity] });
  assert.deepEqual([...empty.scale], [0, 2, 5, 7, 10]);
  assert.deepEqual([...invalid.scale], [0, 2, 5, 7, 10]);
});

test("table generation supports ranges outside the original fixed octaves", () => {
  const q = new ScaleQuantizer({ minimum: 108, maximum: 120 });
  assert.ok(q.table.length > 0);
  assert.ok(q.table.every((midi) => midi >= 108 && midi <= 120));
});

test("reversed range bounds are normalized", () => {
  const q = new ScaleQuantizer({ minimum: 72, maximum: 36 });
  assert.ok(q.table.every((midi) => midi >= 36 && midi <= 72));
  assert.equal(q.foldMidi(84, 72, 36), 72);
});

test("non-finite pitch inputs use deterministic tonic fallbacks", () => {
  const q = new ScaleQuantizer({ tonicMidi: 41 });
  assert.equal(q.quantizeMidi(NaN), 41);
  assert.equal(q.degreeToMidi(Infinity), 41);
  assert.ok(Number.isFinite(midiToHz(NaN)));
});
