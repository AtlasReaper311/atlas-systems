...
const MOD = "/static/js/spectral-forge/prototypes/field-proto-ferrosphere.js";
...
async function metrics() {
  return page.evaluate(async () => {
    ...
    return { raf, litPct: Math.round((lit / total) * 100), maxL, renderer: c.dataset.fieldRenderer, vis: document.visibilityState };
  });
}
...
console.log("NORMAL", await metrics());
await page.locator(".forge-play .forge-field-stage").screenshot({ path: path.join(OUT, "ferrosphere-normal-3s.png") });
...
console.log("CASCADE", await metrics());
await page.locator(".forge-play .forge-field-stage").screenshot({ path: path.join(OUT, "ferrosphere-cascade-20s.png") });
await browser.close();
