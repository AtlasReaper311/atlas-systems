...
export const F31_FIELD_COUNT = 7;

function shaderPrelude() {
  return `
#define ATLAS_F31_FIELD_COUNT ${F31_FIELD_COUNT}
uniform vec4 uAtlasFields[ATLAS_F31_FIELD_COUNT];
uniform vec4 uAtlasFieldParamsA[ATLAS_F31_FIELD_COUNT];
uniform vec4 uAtlasFieldParamsB[ATLAS_F31_FIELD_COUNT];
...
vec3 atlasF31Displaced(vec3 source, out float stressOut, out float routeOut, out float microOut) {
  vec3 p = normalize(source);

  float identity = sin(p.x * 2.1 + p.y * 1.7 - p.z * 1.35 + uAtlasIdentitySeed) * 0.018
    + cos(p.x * 1.15 - p.y * 2.35 + p.z * 1.8 - uAtlasIdentitySeed * 0.7) * 0.013;
  float breath = (uAtlasBreathing - 0.5) * (0.012 + uAtlasDisplacement * 0.012);
  ...
  float ridgeScale = (0.015 + uAtlasDisplacement * 0.022 + uAtlasDamage * 0.018) * uAtlasAudioEnergy;

  for (int i = 0; i < ATLAS_F31_FIELD_COUNT; i++) {
    ...
    radial += signedEnergy * uAtlasAudioEnergy;
    ...
    if (ring > 0.0005) {
      ...
      float pull = ring * fieldFlow * polarity * uAtlasAudioEnergy;
      flow += tangent * pull;
      ...
      if (crestGate > 0.001 && polarity > 0.0) {
        ...
        radial += influence3 * crestGate * crestPulse * crest * uAtlasAudioEnergy;
      }
    }
  }

  float conflict = atlasSat(overlap / float(ATLAS_F31_FIELD_COUNT) * 2.25);
  ...
  radial += conflict * conflictWave
    * (0.026 + uAtlasPhaseDisagreement * 0.044 + uAtlasDamage * 0.04)
    * uAtlasAudioEnergy;
  radial += gather * conflict * (0.022 + uAtlasDisplacement * 0.036);
  ...
  radial += counterA * counterB
    * (0.026 + uAtlasDisplacement * 0.032 + uAtlasActivity * 0.018)
    * uAtlasAudioEnergy;

  float mesoA = sin(dot(p, vec3(8.6, -7.2, 6.9)) + uAtlasPhase * (0.61 + uAtlasActivity * 0.17));
  float mesoB = cos(dot(p, vec3(-6.3, 9.8, 7.7)) - uAtlasPhase * 0.47 + mesoA * 0.38);
  float mesoC = sin(dot(p, vec3(11.7, 5.4, -8.5)) + uAtlasPhase * 0.36 + mesoB * 0.44);
  float meso = mesoA * mesoB * 0.55 + mesoC * 0.45;
  float mesoGate = atlasSat(0.14 + gradient * 2.1 + conflict * 0.42 + uAtlasDisplacement * 0.18);
  radial += meso * mesoGate
    * (0.006 + uAtlasDisplacement * 0.014 + uAtlasDamage * 0.012 + uAtlasPhaseDisagreement * 0.008);
  ...
  float cluster = smoothstep(
    0.49,
    0.77,
    clusterA * 0.42 + clusterB * 0.34 + clusterC * 0.24 + conflict * 0.22 + gradient * 0.32
  );
  ...
  float cell = s1 * s2 * s3;
  float spike = smoothstep(0.31, 0.67, cell + uAtlasMicrostructure * 0.055 + conflict * 0.055);
  spike = spike * spike;
  spike *= spike;
  ...
  float fineSpike = smoothstep(0.52, 0.86, fineCell + uAtlasMicrostructure * 0.035 + gradient * 0.06);
  ...
  float microAmp = (
      spike * (0.018 + uAtlasMicrostructure * 0.052 + uAtlasDamage * 0.032 + uAtlasPhaseDisagreement * 0.018)
      + fineSpike * (0.006 + uAtlasMicrostructure * 0.019 + uAtlasDamage * 0.012)
    ) * microGate * microAudio;
  radial += microAmp;

  float radius = 1.0 + radial;
  vec3 displaced = vec3(
    p.x * radius * (1.07 + uAtlasLateralSpread * 0.045 + uAtlasDamage * 0.02),
    p.y * radius * (0.91 - uAtlasCompression * 0.025 + uAtlasStretch * 0.02),
    p.z * radius * (0.96 + uAtlasAfterimage * 0.018)
  );

  float flowScale = 0.92 + uAtlasActivity * 0.22 + uAtlasDamage * 0.16;
...
function replaceRequired(source, anchor, replacement) {
  if (!source.includes(anchor)) {
    throw new Error(`Anatomy F3.1 shader anchor missing: ${anchor}`);
  }
  return source.replace(anchor, replacement);
}

export function createF31UniformState() {
  return {
    fields: Array.from({ length: F31_FIELD_COUNT }, () => new THREE.Vector4()),
    fieldParamsA: Array.from({ length: F31_FIELD_COUNT }, () => new THREE.Vector4()),
    fieldParamsB: Array.from({ length: F31_FIELD_COUNT }, () => new THREE.Vector4()),
    phase: { value: 0 },
    activity: { value: 0 },
    damage: { value: 0 },
...
export function configureF31Material(material, uniforms, perf) {
  material.customProgramCacheKey = () => "atlas-spectral-forge-anatomy-f31-gpu-v1";
  material.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = replaceRequired(
      ...
      `#include <color_fragment>\nvec3 atlasCool = vec3(0.012, 0.014, 0.026) * vAtlasStress + vec3(0.018, 0.021, 0.038) * vAtlasMicro;\ndiffuseColor.rgb += atlasCool;\nfloat atlasWound = clamp(vAtlasRoute, 0.0, 1.0);\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.30, 0.105, 0.012), atlasWound * 0.42);`,
    );
